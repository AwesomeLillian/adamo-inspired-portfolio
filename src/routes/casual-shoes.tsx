...
const casualOptions = [
  { image: casualOption1.url, alt: "Black brogue lace-up shoe with a chunky sole" },
  { image: casualOption2.url, alt: "Polished black derby shoe with a chunky sole" },
  { image: casualOption3.url, alt: "Black double monk-strap shoe with a contrast welt" },
  { image: casualOption4.url, alt: "Black tassel loafer with a contemporary sole" },
  { image: casualOption5.url, alt: "Textured black lace-up moc-toe shoe" },
  { image: casualOption6.url, alt: "Black penny loafer with a rugged sole" },
  { image: casualOption7.url, alt: "Black textured slip-on shoe with a white sole" },
  { image: casualOption8.url, alt: "Navy suede casual lace-up shoe" },
  { image: casualOption9.url, alt: "Black leather cap-toe derby shoe" },
];
      ...
      <section className="border-y border-border bg-card">
        <div className="mx-auto max-w-7xl px-6 py-24">
          ...
          <div className="mt-4 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <h2 className="max-w-2xl font-display text-3xl font-extrabold uppercase tracking-tight text-foreground md:text-5xl">Explore our casual shoes</h2>
            <p className="max-w-md text-sm leading-relaxed text-muted-foreground">Browse our current selection, then visit us in store to find your preferred style and size.</p>
          </div>
          <div className="mt-12 grid grid-cols-2 gap-3 md:gap-5 lg:grid-cols-4">
            {casualOptions.map((option) => (
              <figure key={option.image} className="group overflow-hidden rounded-md border border-border bg-background">
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={option.image} alt={option.alt} loading="lazy" width={1366} height={768} className="card-zoom h-full w-full object-cover" />
                </div>
              </figure>
            ))}
          </div>
